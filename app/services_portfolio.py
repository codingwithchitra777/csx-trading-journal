...
class PortfolioService:
    ...
    def unrealised_pnl(self, user_id: str, ticker: str, current_price: int) -> Dict[str, Any]:
        ...
        # Weighted average cost of remaining lots (incl. proportional commission)
        ...
        for l in remaining_lots:
            buy_qty = l["qtyOpen"]
            # approx unit cost: buy price + (commission / original buy qty) is better,
            # but we only stored commission and open qty here; acceptable approximation:
            # allocate commission by open qty proportion in that lot.
            # If you want exact, store original buyQty in remaining_lots too.
            unit_cost = float(l["price"])  # keep simple
